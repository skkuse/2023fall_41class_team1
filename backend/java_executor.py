    ...
    try:
        ...
        for file_name in file_names:
            ...
            if compile_result.returncode != 0:
                raise Exception(f"Compilation error in {file_name}: {compile_result.stderr}")
        ...
        execute_result = subprocess.run(['java', main_class_name], capture_output=True, text=True)
        end_time = time.time()
